...
class PythonHighlighter(QSyntaxHighlighter):
    ...
    def define_rules(self):
        ...
        self._rules.append((r"\b[A-Za-z0-9_]+(?=\()", self._formats["function"]))
...
class CodeEditor(QTextEdit):
    """QTextEdit mejorado con numeración de líneas."""
    ...
    def lineNumberAreaWidth(self):
        ...
        width = 3 + self.fontMetrics().horizontalAdvance('9') * digits
        return width
...
class EdgeItem(QGraphicsLineItem):
    ...
    def update_position(self):
        ...
        dest_rect = self.dest_item.rect()

        source_center = self.source_item.pos() + source_rect.center()
        dest_center = self.dest_item.pos() + dest_rect.center()
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.current_project_dir = self.default_scripts_dir
        self.registry = {}
        ...
        self.load_project(os.path.join(self.default_scripts_dir, "scripts"))
    ...
    def create_menus_and_toolbars(self):
        ...
        refresh_action.triggered.connect(self.load_registry)
    ...
    def on_open_project(self):
        ...
        if dialog.exec():
            ...
            from main import open_windows # Importamos la lista de ventanas aquí para evitar el error de importación circular
            new_window = MainWindow()
            new_window.load_project(selected_dir)
            open_windows.append(new_window)
            new_window.show()

    def load_project(self, directory):
        """Carga un nuevo directorio de proyecto y actualiza la UI."""
        self.current_project_dir = directory
        # Establece la raíz del modelo de archivos directamente al directorio de scripts.
        self.file_model.setRootPath(directory)
        self.file_tree.setRootIndex(self.file_model.index(directory))
        self.load_registry(directory)
    ...
    def build_graph_view(self):
        ...
        if not self.graph.nodes:
            self.code_editor.setPlainText("No hay scripts en el directorio. Usa 'Abrir Proyecto' para cargar.")
            return
    ...
    def load_registry(self, directory):
        """Recarga el registro de scripts y reconstruye el grafo."""
        self.registry = discover_scripts(directory)
        self.state_manager = StateManager(self.registry)
    ...
    def run_scheduled_scripts(self):
        ...
        for script_config in self.scheduled_scripts:
            ...
            # Chequeo para la frecuencia 'Diario'
